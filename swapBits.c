...
unsigned int swapBits(unsigned int number, int p1, int p2, int n)
{
	...
	int bin[32]={0}, int corr[32]={0}, num;
     int rem, mul=1, index, index1, temp, ans=0;
     num=number;
     ...
     for(index1=index,index2=0;indx2>0;index2++, index1--)
     {
          corr[index2]=bin[index1];

     }
     for (index=0;index<=32;index++)
     {
          ans=corr[index]*mul+ans;
          mul=mul*2;
